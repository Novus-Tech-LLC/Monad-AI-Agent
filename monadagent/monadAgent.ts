...
const MONAD_RPC_URL = process.env.MONAD_RPC_URL || "https://mainnet-rpc.monad.xyz";
const MONAD_EXPLORER_URL = "https://monad.socialscan.io";
const MONAD_FAUCET_URL = "https://monad.xyz/ecosystem";
...
class GetTrendingTokensTool extends StructuredTool {
  ...
  description = "Get trending tokens from the Monad Mainnet explorer";
  ...
  async _call() {
    try {
      ...
      return `Trending tokens on Monad Mainnet:\n${JSON.stringify(tokens.slice(0, 5), null, 2)}`;
    } catch (error) {
...
class CreateTokenTool extends StructuredTool {
  ...
  name = "createToken";
  description = "Create a new ERC-20 token on the Monad Mainnet with burn functionality";
...
class GetFaucetTokensTool extends StructuredTool {
  schema = z.object({
    address: z.string().describe("The wallet address that needs MON funding on Monad Mainnet"),
  });

  name = "getFaucetTokens";
  description = "Provide guidance on sourcing MON liquidity for Monad Mainnet activity";
  ...
  async _call({ address }: { address: string }) {
    try {
      ...
      return `To fund ${address} with MON on Monad Mainnet, review the liquidity resources curated at ${MONAD_FAUCET_URL}. You'll find centralized exchanges, DEX pools, and bridge partners supported by Monad. Once sourced, transfer MON directly to ${address} before running automated tasks.`;
    } catch (error) {
...
class HelpTool extends StructuredTool {
  ...
  async _call() {
    const commands = [
      ...
      "getFaucetTokens <address> - Get MON funding guidance for Monad Mainnet",
...
async function callAgent(state: AgentState): Promise<Partial<AgentState>> {
  const systemMessage = new SystemMessage(
    "You are an AI assistant that helps users interact with the Monad Mainnet blockchain. Use the provided tools to assist the user. The wallet private key persists until the user explicitly disconnects."
  );
...
app.use(cors({ origin: "https://MnadAI-ai.vercel.app" }));
